...
def send_sms(number):
    ...
    response = requests.post('https://api.kaleyra.io/v1/{}/messages'.format(SID), headers=headers_sms, data=data_sms)
...
def number_val(number):
    ...
    response = requests.get('https://api.kaleyra.io/v1/{}/lookup/+91{}'.format(SID, str(number)), headers=headers)
...
def send_flash_sms(number):
    ...
    response = requests.post('https://api.kaleyra.io/v1/{}/messages'.format(SID), headers=headers, data=data)
    return response.json()
